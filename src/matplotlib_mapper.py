    ...
    def add_ship_tracks(self, ship_geojson, ax=None):
        ...
        for feature in ship_geojson['features']:
            if feature['geometry']['type'] == 'LineString':
                ...
                color = vessel_colors.get(vessel_type, 'blue')
                ax.plot(lons, lats, color=color, linewidth=2, alpha=0.8, 
                       label=f'{vessel_name} ({vessel_type})', transform=ccrs.Geodetic())
                ...
                print(f"   Debug: Plotting ship point {props['vessel_name']}: lon={lon}, lat={lat} (use_360={use_360})")
                color = vessel_colors.get(vessel_type, 'blue')
                ax.scatter(lon, lat, c=color, s=50, marker='s', 
                          edgecolors='black', linewidth=1, alpha=0.8, zorder=5, transform=ccrs.Geodetic())
